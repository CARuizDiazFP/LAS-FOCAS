...
# Descripción: Skill para sincronizar el repositorio con validación documental, commit técnico y push a dev (rama de trabajo habitual) — mirror de .agentes-comunes/skills/repo-updater/SKILL.md (fuente de verdad)
...
argument-hint: "Describe alcance o contexto, por ejemplo: sincronizar cambios de web y docs a dev"
---
...
- sincronizar cambios locales con `dev` (rama de trabajo habitual — nunca `main` ni un worktree de prod)
...
1. Confirmar el worktree/rama de destino ANTES de tocar nada: `git rev-parse --abbrev-ref HEAD` debe
   ser `dev`, y `git rev-parse --show-toplevel` debe ser el checkout real de trabajo del proyecto, no
   un worktree aislado (de investigación, de un agente en `isolation: "worktree"`, o de prod). Ver
   guardrail 6 — esto es una regla dura, no una preferencia.
...
8. Ejecutar `git add` (de los archivos/hunks que correspondan a este commit — nunca `git add .` a
   ciegas si el paso 3 detectó una mezcla), `git commit` y `git push origin dev`.
...
6. **Regla dura, confirmada explícitamente por el usuario (2026-08-25)**: este skill siempre commitea
   en el worktree de `dev` — nunca en un worktree de prod, ni en uno creado para una investigación o
   tarea aislada (`superpowers:using-git-worktrees`, `isolation: "worktree"` de un agente, `EnterWorktree`).
   Los prompts e investigaciones de este proyecto usan siempre la rama `dev`; si el directorio actual
   no es ese worktree, detenerse y decírselo al usuario en vez de commitear donde sea que se esté
   parado.

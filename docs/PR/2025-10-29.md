...
- Nuevo endpoint `POST /api/reports/sla` en `web_app` que admite 0-2 archivos y reutiliza `core.services.sla`.
...
## Cambios realizados
- `web/web_app/main.py`: agrega GET `/sla`, helper para combinar hasta dos Excel y POST `/api/reports/sla` reutilizando el servicio SLA.
...
- Se mantiene autenticación de sesión + CSRF para `POST /api/reports/sla`.

...
1. Crear tests unitarios para las nuevas funciones del parser de tracking
...
5. ~~Corregir cálculo de horas SLA (columna U vs P)~~ ✅
6. Implementar endpoint `/api/infra/notify/send` para envío de correos SMTP
...
## Etapa 6: Corrección Crítica del Cálculo de Horas SLA

### Problema Identificado

El informe de SLA mostraba valores incorrectos de "Horas Netas" para los reclamos. Al comparar con el Excel fuente se detectó que:

- El informe mostraba valores de **columna P** ("Horas Netas Cierre Problema Reclamo")
- Debía mostrar valores de **columna U** ("Horas Netas Reclamo")

**Ejemplo de discrepancia detectada:**

| Servicio | Excel Col U | Informe Mostraba |
|----------|-------------|------------------|
...
En `core/sla/legacy_report.py`, la función `_columna_horas_reclamos()` retornaba:
...
```python
# CORRECTO:
return columns.get("horas")  # Columna U ("Horas Netas Reclamo")
```
...
| Archivo | Cambio |
|---------|--------|
| [core/sla/legacy_report.py](core/sla/legacy_report.py#L133) | `_columna_horas_reclamos()` ahora usa `columns.get("horas")` |
| [core/sla/parser.py](core/sla/parser.py#L45) | Eliminada columna fallback `"Horas Netas Cierre Problema Reclamo"` |
| [modules/informes_sla/processor.py](modules/informes_sla/processor.py#L17) | Docstring actualizado documentando uso de columna U |
| [tests/test_sla_legacy_report.py](tests/test_sla_legacy_report.py) | Test actualizado para validar columna correcta |

### Mapeo de Columnas SLA (Referencia)

```python
RECLAMOS_REQUIRED = {
    "id_reclamo": ["Número reclamo"],
    "id_servicio": ["Número Línea", "Número Primer Servicio"],
    "cliente": ["Razón Social", "Cliente"],
    "horas": ["Horas Netas Reclamo"],  # ← Columna U del Excel
}
```

### Validación

Se verificó con el archivo real `Reclamos SLA - Diciembre 2025.xlsx`:
...
- [x] `_columna_horas_reclamos()` retorna columna correcta ("horas" = Col U)
- [x] Parser SLA no tiene columnas fallback incorrectas
- [x] Tests actualizados y pasando
- [x] Informe genera valores coherentes con Excel fuente
- [x] Docker rebuildeado y validado

<!--
  ...
  Descripción: Vista admin de ingesta automática de Excel de servicios SLA con barra de progreso
-->
<template>
  <section class="admin-ingesta">
    <h1>Ingesta</h1>
    <p class="section-subtitle">Carga automática del Excel de Servicios SLA (entorno dev).</p>
    ...
    <article class="card ingesta-card">
      ...
      <label class="ingesta-card__dropzone" for="file-input">
        <span>Seleccioná el archivo de Servicios SLA</span>
        <small>Formatos soportados: .xlsx, .xlsm, .csv</small>
      </label>
      <input id="file-input" ref="fileInput" type="file" accept=".xlsx,.xlsm,.csv" hidden @change="onSelectFile" />

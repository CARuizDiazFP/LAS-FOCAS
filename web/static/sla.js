...
(function(){
  ...
  function updateDropLabel(files){
    ...
    if (files.length === 1){
      label.textContent = `Seleccionado: ${files[0].name}`;
      return;
    }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!useDbChecked){
      if (!files.length){
        setFlash('Adjuntá al menos un archivo .xlsx o activá “Usar base”.', 'error');
        return;
      }
      if (files.length > 2){
        setFlash('Solo se permiten hasta dos archivos .xlsx.', 'error');
        return;
      }
    }
    ...
    try {
      ...
      if (!response.ok || !data || data.ok === false){
        const errorMessage = data && (data.error || data.detail || data.message);
        throw new Error(errorMessage || 'No se pudo generar el informe');
      }
      setFlash(data.message || 'Informe SLA generado correctamente.', 'success');
      renderResults(data.report_paths);
    } catch (error) {
      setFlash(`Error: ${error.message || error}`, 'error');
      clearResults();
    }
  });
})();
